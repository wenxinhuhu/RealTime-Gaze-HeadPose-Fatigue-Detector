...
class BasicBlock(nn.Module):  # 适合 resnet 18,34
    expansion = 1
...
class Bottleneck(nn.Module):  # 适合 resnet 50,101,152
    expansion = 4
    def __init__(self, inplanes, planes, stride=1, downsample=None):
...
# p_num = sum([p.numel() for p in resnet18.parameters()])   #11.19M
...
# p_num = sum([p.numel() for p in resnet152.parameters()])   #11.19M
# print("Number of parameter: %.2fM" % (p_num / 1e6))  # 统计模型参数
# x = torch.randn(32,3,224,224)
